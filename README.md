...
结合此前的自回归缩放实验、当前 MLM 结果与 REAL-LONG Cue 消融，**目前未发现 HCAM 相比纯 Transformer 存在可明确归因于混合卷积-注意力架构本身的系统性性能差异或明显精度损失。** 换句话说，现有结果没有显示“将大部分注意力层替换为门控扩张卷积”本身造成了明显能力退化；严格区分架构因素仍需要未来进行同数据、同参数量、同训练步数的控制实验。
