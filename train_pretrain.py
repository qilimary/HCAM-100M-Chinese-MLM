...
10-virtual-epoch masking schedule, AdamW settings, token-based LR schedule,
AMP/DDP, gradient accumulation, validation, and checkpoint/export format.

Important schedule note:
The 10 virtual epochs are scheduling segments, NOT 10 full corpus passes.
Virtual epochs 1-5 jointly cover one full pass with the original 600-token
window alignment. Virtual epochs 6-10 jointly cover a second full pass using
a half-window (300-token) offset. The released run therefore corresponds to
about two corpus passes in total (~3.276B character-token exposures for a
~1.638B-character corpus), not ten passes.
...
VIRTUAL_EPOCHS_PER_PASS = 5
RELEASED_VIRTUAL_EPOCHS = 10
...
class TrainConfig:
    seed: int = 42
    seq_len: int = 600
    virtual_epochs: int = RELEASED_VIRTUAL_EPOCHS
    ...
    # Original virtual epoch 10 used an LR floor of 1e-4.
...
# -------------------------------------------------------------------------------------------------
# Memory-mapped sequence dataset and samplers
# -------------------------------------------------------------------------------------------------
...
class DistributedSequentialSampler(Sampler[int]):
    """Validation sampler. Uneven rank lengths are safe because evaluation uses raw model."""
...
def virtual_partition_bounds(n: int, part_index: int, parts: int = VIRTUAL_EPOCHS_PER_PASS) -> tuple[int, int]:
    if not (0 <= part_index < parts):
        raise ValueError(f"part_index must be in [0,{parts}), got {part_index}")
    start = (n * part_index) // parts
    end = (n * (part_index + 1)) // parts
    return start, end


class DistributedVirtualEpochSampler(Sampler[int]):
    """Non-overlapping 1/5 corpus slice for one virtual epoch.

    A deterministic global permutation is generated once per full corpus pass.
    The permutation is split into five disjoint parts. Therefore virtual epochs
    1-5 jointly see every base-alignment sequence once, and virtual epochs 6-10
    jointly see every offset-alignment sequence once.

    For DDP training, each part is padded by at most world-1 indices so every
    rank has exactly the same number of samples/steps. This avoids a possible
    DDP hang on non-divisible shard sizes. The padding overhead is negligible.
    """

    def __init__(self, n: int, part_index: int, pass_index: int, rank: int, world: int,
        ...
        self.start, self.end = virtual_partition_bounds(self.n, self.part_index)
        self.raw_part_len = self.end - self.start
        if self.raw_part_len > 0 and self.world > 1:
            self.padded_part_len = math.ceil(self.raw_part_len / self.world) * self.world
        else:
            self.padded_part_len = self.raw_part_len

    def __iter__(self):
        if self.raw_part_len <= 0:
            return iter([])
        indices = np.arange(self.n, dtype=np.int64)
        if self.shuffle:
            # Same permutation for all five virtual epochs belonging to one pass.
            rng = np.random.default_rng(self.seed + self.pass_index * 1_000_003)
            rng.shuffle(indices)
        subset = indices[self.start:self.end]
        if self.padded_part_len > self.raw_part_len:
            pad = self.padded_part_len - self.raw_part_len
            subset = np.concatenate([subset, np.resize(subset, pad)])
        return iter(subset[self.rank::self.world].tolist())

    def __len__(self):
        if self.world <= 1:
            return self.raw_part_len
        return self.padded_part_len // self.world


def planned_virtual_epoch_sequences(n_first: int, n_second: int, virtual_epochs: int, world: int) -> int:
    total = 0
    for epoch_idx in range(virtual_epochs):
        if epoch_idx < VIRTUAL_EPOCHS_PER_PASS:
            n = n_first
            part_index = epoch_idx
        else:
            n = n_second
            part_index = epoch_idx - VIRTUAL_EPOCHS_PER_PASS
        start, end = virtual_partition_bounds(n, part_index)
        size = end - start
        if size > 0 and world > 1:
            size = math.ceil(size / world) * world
        total += size
    return total
...
def strategy_for_epoch(cfg: TrainConfig, epoch_number: int):
    if epoch_number <= 3:
        return cfg.mask_strategy_1_3
    if epoch_number <= 6:
        return cfg.mask_strategy_4_6
    if epoch_number <= 9:
        return cfg.mask_strategy_7_9
...
def save_training_checkpoint(path: Path, raw_model: HCAM, optimizer, scaler, cfg: TrainConfig,
    ...
    payload = {
        ...
        # Keep 'epoch' for compatibility; it means completed virtual epochs.
        "epoch": epoch,
        "virtual_epoch": epoch,
        "completed_full_passes": epoch // VIRTUAL_EPOCHS_PER_PASS,
...
def save_final_model(path: Path, raw_model: HCAM, cfg: TrainConfig, epoch: int, global_step: int,
                     exposed_tokens: int, tokenizer_sha256: str, val_loss: float,
                     val_acc: float, best_val: float):
    payload = {
...
def train(args):
    ...
    if not (1 <= cfg.virtual_epochs <= RELEASED_VIRTUAL_EPOCHS):
        raise ValueError(
            f"--epochs is the number of virtual epochs and must be in [1,{RELEASED_VIRTUAL_EPOCHS}] "
            f"for the released two-pass recipe; got {cfg.virtual_epochs}"
        )
    ...
    if args.resume:
        ...
        start_epoch = int(ckpt.get("virtual_epoch", ckpt.get("epoch", 0)))
    ...
    # Two full corpus passes, each split into five non-overlapping virtual epochs.
    # Pass 1: original sequence alignment. Pass 2: half-window offset alignment.
    base_train = StreamDataset(args.train_prefix, cfg.seq_len, offset=0,
                               max_sequences=args.max_train_sequences)
    offset_train = StreamDataset(args.train_prefix, cfg.seq_len, offset=cfg.seq_len // 2,
                                 max_sequences=args.max_train_sequences)
    val_ds = StreamDataset(args.val_prefix, cfg.seq_len, offset=0,
    ...
    planned_sequences = planned_virtual_epoch_sequences(
        n_first, n_second, cfg.virtual_epochs, world
    )
    total_exposure_tokens = max(1, cfg.seq_len * planned_sequences)
    ...
    rank0_print(rank, f"virtual_epochs={cfg.virtual_epochs}; 5 virtual epochs = 1 full corpus pass")
    rank0_print(rank, f"base_sequences={n_first:,} offset_sequences={n_second:,}")
    ...
    for epoch_idx in range(start_epoch, cfg.virtual_epochs):
        ...
        sampler = DistributedVirtualEpochSampler(
            len(ds), part_index=part_index, pass_index=pass_index,
            rank=rank, world=world, seed=cfg.seed, shuffle=True,
        )
        ...
        if loader_steps == 0:
            raise RuntimeError(
                f"virtual epoch {epoch_num} contains zero batches; increase --max-train-sequences "
                f"or use a larger prepared stream"
            )
        ...
        for step, (clean, guide) in enumerate(loader, 1):
            ...
            # The training sampler pads each virtual shard to an exact multiple of world,
            # so all ranks process the same number of examples and this global counter is exact.
            ...
            with sync_ctx:
                with torch.autocast(device_type=device.type, dtype=torch.float16,
                    ...
                    logits = model(corrupted, mask_embedding_positions=mask_pos,
                                   prediction_indices=indices)
                    loss, ce, z, correct, count = masked_loss(
                        logits, targets, valid, cfg.z_loss_weight
                    )
            ...
            if rank == 0 and (step % args.log_every == 0 or step == loader_steps):
                ...
                print(
                    f"VE{epoch_num}/{cfg.virtual_epochs} pass={pass_index + 1}/2 "
                    f"part={part_index + 1}/5 {step}/{loader_steps} "
        ...
        last_val_loss, last_val_acc = evaluate(
            model, val_loader, tok, cfg, device, distributed, rank
        )
        if rank == 0:
            best_val = min(best_val, last_val_loss)
            print(
                f"Virtual epoch {epoch_num} complete | val_loss={last_val_loss:.4f} "
                f"val_acc={last_val_acc:.2%} masked_ppl={math.exp(min(20,last_val_loss)):.2f} "
                f"exposed_tokens={exposed_tokens:,}",
                flush=True,
            )
    ...
    if rank == 0:
        save_final_model(
            final_path, raw_model, cfg, cfg.virtual_epochs, global_step, exposed_tokens,
            tokenizer_sha, last_val_loss, last_val_acc, best_val,
        )
...
def build_parser():
    ...
    tr.add_argument(
        "--epochs", type=int, default=RELEASED_VIRTUAL_EPOCHS,
        help="number of virtual epochs (released recipe: 10; every 5 virtual epochs = one full corpus pass)",
    )
